...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
  ...
  <xsl:template match="breadcumbs">
    <a class="xsl_file" href="{$vlozhen}index.html" target="_top">Справка</a> ->
  ...
  <xsl:template match="objLink" name="objLink">
    <a class="xsl_file">
      <xsl:attribute name="href">
        <xsl:value-of select="$vlozhen"/><xsl:value-of select="$useful"/>Objects/<xsl:value-of select="$objEng"/>.html
      </xsl:attribute>
      <xsl:value-of select="$obj"/>
    </a>
  </xsl:template>
  ...
  <xsl:template match="propsLink">
    <a class="xsl_file">
      <xsl:attribute name="href">
        <xsl:value-of select="$vlozhen"/><xsl:value-of select="$useful"/>Objects/<xsl:value-of select="$objEng"/>/PropertiesCommands.html
      </xsl:attribute>
      Свойства и команды
    </a>
  </xsl:template>
  ...
  <xsl:template match="propLink">
    <a class="xsl_file">
      <xsl:attribute name="href">
        <xsl:value-of select="$vlozhen"/><xsl:value-of select="$useful"/>Objects/<xsl:value-of select="$objEng"/>/PropertiesCommands/<xsl:value-of select="$propEng"/>.html
      </xsl:attribute>
      <xsl:value-of select="$prop"/>
    </a>
  </xsl:template>
  ...
  <xsl:template match="eventsLink">
    <a class="xsl_file">
      <xsl:attribute name="href">
        <xsl:value-of select="$vlozhen"/><xsl:value-of select="$useful"/>Objects/<xsl:value-of select="$objEng"/>/Events.html
      </xsl:attribute>
      События
    </a>
  </xsl:template>
  ...
  <xsl:template match="eventLink">
    <a class="xsl_file">
      <xsl:attribute name="href">
        <xsl:value-of select="$vlozhen"/><xsl:value-of select="$useful"/>Objects/<xsl:value-of select="$objEng"/>/Events/<xsl:value-of select="$propEng"/>.html
      </xsl:attribute>
      <xsl:value-of select="$prop"/>
    </a>
  </xsl:template>
      ...
      <h4 class="xsl_file">
        <span class="xsl_file_spanLink" onClick="javascript:SwapDisplay('xsl_file_divExceptions','xsl_file_imgExceptions');">
          ...
          <xsl:if test="not(@type)">
            Данные, требуемые для выполнения свойсва или команды:
          </xsl:if>
