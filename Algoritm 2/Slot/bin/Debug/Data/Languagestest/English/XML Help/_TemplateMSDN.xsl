      ...
      <BODY class="xsl_file">
        ...
        <div class="xsl_file_header">
          ...
          <xsl:if test="file/seealso/est">
            <a class="xsl_file" href="#see">See also</a>
          </xsl:if>
          <xsl:if test="file/samples/sample">, <a href="#ex" class="xsl_file">Sample</a>
          </xsl:if>
        </div>
        <xsl:if test="file/seealso or file/samples or file/exceptions or file/remarks or file/signature or file/descriptionarticle">
          <div class="xsl_file_header1">
            <span class="xsl_file_spanLink" onClick="javascript:ChColExpAll('xsl_file_spColexpAll', 'xsl_file_imgColexpAll')">
              ...
              <span id="xsl_file_spColexpAll">Collapse all</span>
            </span>
          </div>
        </xsl:if>
        <div>
  ...
  <xsl:template match="descriptionarticle">
    <h4 class="xsl_file">
      <span class="xsl_file_spanLink" onClick="javascript:SwapDisplay('xsl_file_divSyntax','xsl_file_imgSyntax');">
        <img id="xsl_file_imgSyntax" src="{$vlozhen}{$msdnTemp}/minus.gif" border="0"/> Description
      </span>
    </h4>
    <div>
  ...
  <xsl:template match="childlist">
    <xsl:if test="count(child)>0">
      <xsl:if test="@type='property'">
        List of object properties <xsl:call-template name="objLink"></xsl:call-template>:
      </xsl:if >
 ...
 <xsl:template match="blockquote">
  ...
  <xsl:template match="signature">
    <xsl:if test="./*/name != ''">
      <h4 class="xsl_file">
        <span class="xsl_file_spanLink" onClick="javascript:SwapDisplay('xsl_file_divSyntax','xsl_file_imgSyntax');">
          <img id="xsl_file_imgSyntax" src="{$vlozhen}{$msdnTemp}/minus.gif" border="0"/> Syntax
        </span>
      </h4>
  ...
  <xsl:template match="parameters">
    <xsl:if test="count(parameter) &gt; 0">
      <h5 class="xsl_file">The data to be transferred to</h5>
  ...
  <xsl:template match="exceptions">
    <xsl:if test="count(exception) &gt; 0">
      <h4 class="xsl_file">
        <span class="xsl_file_spanLink" onClick="javascript:SwapDisplay('xsl_file_divExceptions','xsl_file_imgExceptions');">
          ...
          <xsl:if test="not(@type)">
            The data required to implement properties or commands:
          </xsl:if>
          <xsl:if test="@type='eventProperties'">
            Additional features which appear at the sites at this event:
          </xsl:if>
        </span>
      </h4>
      <div id="xsl_file_divExceptions" class="xsl_file_container">
        ...
        <xsl:if test="@type='eventProperties'">
           Object that happened on the event, adds a happy <b> new features </b> (for example, X and Y mouse, 
           key pressed). These properties can not be changed, but you can see them (because they can not be changed, then find the 
           properties can be <a href="{$vlozhen}Environment/Master.html"> Wizard complex action </a>). 
           Below are all the properties of the event.
          <br/>
        </xsl:if>
    ...
    <xsl:if test="text() or count(./*)>0">
      <h4 class="xsl_file">
        <span class="xsl_file_spanLink" onClick="javascript:SwapDisplay('xsl_file_divRemarks','xsl_file_imgRemarks');">
          <img id="xsl_file_imgRemarks" src="{$vlozhen}{$msdnTemp}/minus.gif" border="0"/> Remark
        </span>
      </h4>
      <div>
    ...
    <xsl:if test="count(sample) &gt; 0">
      ...
      <h4 class="xsl_file">
        <span class="xsl_file_spanLink" onClick="javascript:SwapDisplay('xsl_file_divExample','xsl_file_imgExample');">
          <img id="xsl_file_imgExample" src="{$vlozhen}{$msdnTemp}/minus.gif" border="0"/> Example
        </span>
      </h4>
    ...
    <xsl:if test="count(est) &gt; 0">
      ...
      <h4 class="xsl_file">
        <span class="xsl_file_spanLink" onClick="SwapDisplay('xsl_file_divSeealso','xsl_file_imgSeealso');">
          <img id="xsl_file_imgSeealso" src="{$vlozhen}{$msdnTemp}/minus.gif" border="0"/> See also
        </span>
      </h4>
